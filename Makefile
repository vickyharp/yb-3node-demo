...
YSQL        := ysqlsh -h localhost -p 5433 -U yugabyte
else
...
ifneq ($(shell command -v ysqlsh 2>/dev/null),)
YSQL_TTY    := ysqlsh -h localhost -p 5433 -U yugabyte
...
.DEFAULT_GOAL := help
.PHONY: help up down clean restart status servers connect shell sql demo range-to-hash kill revive logs collect-logs ysqlsh-check
...
logs: ## Tail live logs from all three nodes
...
ysqlsh-check:
	@if ! command -v ysqlsh >/dev/null 2>&1; then \
	  printf "\033[90mysqlsh not found — using ysqlsh inside container.\033[0m\n"; \
